...
exports.createUser = async function (req, res, next) {
    ...
    try {
        ...
        if (createdUser===0){
            ...
            return res.status(400).json({ message: "Error: mail: in use.",repeatedEmail})
        }
        else{
            if (createdUser===1){
                console.log("Error. Usuario no disponible: ==>>",User.user,"<<==");
                var repeatedUser = User.user;
                return res.status(400).json({ message: "Error: user not disponible, in use.",repeatedUser})
            }else{
...
exports.updateUser = async function (req, res, next) {
    ...
    if (!req.body.user) {
        return res.status(400).json({status: 400., message: "User must be present"})
    }
    ...
    try {
        ...
        if (!updatedUser){
            return res.status(400).json({status: 400, data: User, message: "No se encontro el usuario."})
        }
